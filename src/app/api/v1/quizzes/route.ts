...
async function ensureSeededQuizzes() {
  ...
  console.log('Auto-seeding initial HTML quizzes...');
  const adminEmail = 'mail@thestrategist.co.in';
  let trainer = await UserModel.findOne({ email: adminEmail });
  if (!trainer) {
    const passwordHash = await bcrypt.hash('AjayThomas@1', 10);
  ...
  for (const qData of defaultHTMLQuizzes) {
    const questionIds = [];
    ...
    let quiz = await QuizModel.findOne({ title: qData.title });
    if (!quiz) {
