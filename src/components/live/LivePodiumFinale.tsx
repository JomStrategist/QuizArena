...
export const LivePodiumFinale: React.FC<LivePodiumFinaleProps> = ({
  ...
  const myRecord = myIndex >= 0 ? rankings[myIndex] : null;
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-between p-6 md:p-10 space-y-8 font-sans">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-black text-slate-400 uppercase tracking-widest">{quizTitle} FINALE</span>
      </div>

      {/* Student Personal Results Summary Card (If student viewing) */}
      {myRecord && (
        <div className="max-w-2xl mx-auto w-full bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 text-white p-6 rounded-3xl shadow-xl space-y-4">
          <div className="flex items-center justify-between border-b border-white/20 pb-3">
            <div>
              <span className="text-[10px] font-black uppercase tracking-wider text-blue-200">YOUR QUIZ RESULT</span>
              <h2 className="text-xl font-black">{myRecord.displayName}</h2>
            </div>
            <div className="px-3.5 py-1 bg-amber-400 text-slate-950 rounded-full font-black text-xs">
              RANK #{myIndex + 1} OF {rankings.length}
            </div>
          </div>

          <div className="grid grid-cols-3 gap-3 text-center">
            <div className="p-3 bg-white/10 rounded-2xl backdrop-blur-md">
              <p className="text-[10px] font-bold text-blue-200 uppercase">SCORE</p>
              <p className="text-2xl font-black font-mono mt-0.5">{(myRecord.score || 0).toLocaleString()}</p>
            </div>
            <div className="p-3 bg-white/10 rounded-2xl backdrop-blur-md">
              <p className="text-[10px] font-bold text-blue-200 uppercase">CORRECT</p>
              <p className="text-2xl font-black mt-0.5">{myRecord.correctAnswers || 0}</p>
            </div>
            <div className="p-3 bg-white/10 rounded-2xl backdrop-blur-md">
              <p className="text-[10px] font-bold text-blue-200 uppercase">ACCURACY</p>
              <p className="text-2xl font-black mt-0.5">
                {myRecord.correctAnswers && (myRecord.correctAnswers + (myRecord.wrongAnswers || 0)) > 0
                  ? Math.round((myRecord.correctAnswers / (myRecord.correctAnswers + (myRecord.wrongAnswers || 0))) * 100)
                  : 0}%
              </p>
            </div>
          </div>
      ...
      <div className="max-w-3xl mx-auto w-full space-y-8 text-center">
        <div className="space-y-2">
          ...
          <h1 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tight">QUIZARENA CHAMPIONS</h1>
          <p className="text-sm font-bold text-slate-500">Live Game Finale & Rankings</p>
        </div>
        ...
        <div className="flex items-end justify-center gap-3 sm:gap-6 pt-8 pb-4">
          ...
          <div className="flex flex-col items-center space-y-2 w-28 sm:w-36">
            <div className="w-10 h-10 rounded-full bg-slate-200 text-slate-700 flex items-center justify-center font-black text-sm border-2 border-slate-300 shadow-sm">
              2
            </div>
            <p className="text-xs font-extrabold text-slate-800 truncate w-full">{secondPlace.displayName}</p>
            <p className="text-[11px] font-black text-blue-600">{(secondPlace.score || 0).toLocaleString()} pts</p>
          ...
          <div className="flex flex-col items-center space-y-2 w-32 sm:w-44 -mt-6">
            <div className="relative">
              <Crown className="w-7 h-7 text-amber-500 absolute -top-6 left-1/2 transform -translate-x-1/2 animate-pulse" />
            ...
            <p className="text-xs font-black text-amber-600">{(firstPlace.score || 0).toLocaleString()} pts</p>
          ...
          {/* 3rd Place */}
          <div className="flex flex-col items-center space-y-2 w-28 sm:w-36">
            <div className="w-10 h-10 rounded-full bg-amber-700 text-amber-100 flex items-center justify-center font-black text-sm border-2 border-amber-800 shadow-sm">
              3
            </div>
            <p className="text-xs font-extrabold text-slate-800 truncate w-full">{thirdPlace.displayName}</p>
            <p className="text-[11px] font-black text-amber-700">{(thirdPlace.score || 0).toLocaleString()} pts</p>
        ...
        <div className="space-y-2 max-h-48 overflow-y-auto">
          {rankings.map((r, i) => (
            ...
            >
              ...
              <span className="font-black text-blue-600 font-mono">{(r.score || 0).toLocaleString()} pts</span>
            </div>
          ))}
        </div>
      </div>
